...
void Fbx::Exporter::ExportAnimation(wstring saveFolder, wstring fileName)
{
	ReadAnimationData();
}
...
void Fbx::Exporter::ReadAnimationData()
{
	...
	for(int i = 0; i < importer->GetAnimStackCount(); i++)
	{
		...
		float duration = (float)tempDuration.GetSecondDouble();

		if (start < stop)
			ReadKeyframeData(animation, scene->GetRootNode(), start, stop);

		animation->Duration = duration;
		animation->TotalFrame = (stop - start) + 1; //0부터 시작이라서.
...
void Fbx::Exporter::ReadKeyframeData(FbxAnimation * animation, FbxNode * node, int start, int stop)
{
	FbxNodeAttribute* attribute = node->GetNodeAttribute();
	...
	for (int i = 0; i < node->GetChildCount(); i++)
		ReadKeyframeData(animation, node->GetChild(i), start, stop);
}
