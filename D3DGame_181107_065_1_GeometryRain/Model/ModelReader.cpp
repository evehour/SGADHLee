	...
	for (UINT i = 0; i < count; i++)
	{
		...
		for (UINT j = 0; j < partCount; j++)
		{
			...
			//VertexData
			{
				...
				w->UInt(verticeCount);
				w->Byte(&_meshPart->vertices[0], sizeof(VertexTextureNormalTangentBlend) * verticeCount);
			}
...
void Model::WriteMaterial(wstring folder, wstring file)
{
	...
	for (Material* material : materials)
	{
		...
		string dfileName = String::ToString(material->GetDiffuseMapFileName());
		element->SetText(dfileName.c_str());
		node->LinkEndChild(element);
