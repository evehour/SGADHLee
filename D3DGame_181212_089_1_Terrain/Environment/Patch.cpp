...
{
	{
		map<WORD, ID3D11Buffer *>::iterator it;
		for (it = indexBuffer.begin(); it != indexBuffer.end();)
		{
			SAFE_RELEASE(it->second);
			it = indexBuffer.erase(it);
		}
		for (it = centerIndexBuffer.begin(); it != centerIndexBuffer.end();)
		{
			SAFE_RELEASE(it->second);
			it = centerIndexBuffer.erase(it);
		}
	}
	{
...
void Patch::BuildCenterIndices()
{
	...
	{
		WORD t = (WORD)powf(2.0f, tessLevel);
		vector<WORD> indices;
		...
		if (indices.size() > 0)
		{
			...
			desc.MiscFlags = 0; // ResourceOptionFlags == D3D11_RESOURCE_MISC_FLAG
...
void Patch::BuildTopEdges()
{
	...
	for (WORD i = 0; i < 6; i++)
	{
		...
		edgeIndiceCount[NeighborDir::Top][key] = indices.size();
...
void Patch::BuildLeftEdges()
{
	...
	for (WORD i = 0; i < 6; i++)
	{
		...
		edgeIndiceCount[NeighborDir::Left][key] = indices.size();
