	...
	, _position(0, 0, 0), _localForward(0, 0, 1), _localUp(0, 1, 0)
{
	...
	lineShader = new Shader(Shaders + L"052_Gizmo.fx");
...
void GizmoComponent::Update()
{
	...
	Context::GetMainCamera()->Position(&camPosition);
	camDirection = Context::GetMainCamera()->Direction();
	...
	if (_isActive)
	{
		...
		if (Mouse::Get()->Press(0) && ActiveAxis != Axis::None)
		{
			...
			ray.SettingRay(camPosition, camDirection);
	...
	D3DXVECTOR3 vLength = camPosition - _position;
...
void GizmoComponent::Render()
{
	...
	Context::SetValues(lineShader);

	lineShader->AsMatrix("World")->SetMatrix(_gizmoWorld);
				...
				case Axis::XY:
					{
						...
						lineShader->AsVector("DiffuseColor")->SetFloatVector(D3DXCOLOR(0, 0, 0, 0.5f));
						activeQuad->Render(lineShader);
					}
					break;
				#pragma endregion
			}
...
void GizmoComponent::PickObject(bool removeFromSelection)
{
	...
	{
		float intersection;
		ray.SettingRay(camPosition, camDirection, entity->World());
...
void GizmoComponent::SelectAxis()
{
	...
	if (ActiveMode == Mode::Translate)
	{
		ray.SettingRay(camPosition, camDirection, _gizmoWorld);
	}
	else
	{
		D3DXMATRIX matWorld;
