...
Cube::Cube(ExecuteValues * values)
	: Execute(values)
	, m_vec3Position(0, 0, 0), m_vec3Rotate(0, 0, 0), m_vec3Scale(1, 1, 1)
{
...

	{
		...
		fin.open(Contents + L"Model/Cube.txt");
		fin.get(input);
		while (input != ':')
		{
			fin.get(input);
...
void Cube::SetPosition(D3DXVECTOR3 pos)
{
	D3DXMATRIX matWorld;
	m_vec3Position = pos;
	D3DXMatrixTranslation(&matWorld, m_vec3Position.x, m_vec3Position.y, m_vec3Position.z);
