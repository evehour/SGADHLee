...
Context::Context()
{
	...
	//D3DXMatrixOrthoLH(&light.ShadowProject, 1000, 1000, 1, 2000);
	//D3DXMatrixPerspectiveFovLH(&light.ShadowProject, Math::PI * 0.5f, 1, 1, 1000);
...
void Context::ChangeGlobalLight()
{
	...
	//target.x = perFrame.ViewPosition.x;
	//target.z = perFrame.ViewPosition.z;
	...
	float l = centerLs.x - light.Distance;
	float b = centerLs.y - light.Distance;
	float n = centerLs.z - light.Distance;
	float r = centerLs.x + light.Distance;
	float t = centerLs.y + light.Distance;
	float f = centerLs.z + light.Distance;

	D3DXMatrixOrthoOffCenterLH(&light.ShadowProject, l, r, b, t, n, f);
	D3DXMATRIX T;
