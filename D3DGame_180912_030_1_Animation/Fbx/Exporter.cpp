...
void Fbx::Exporter::ExportMesh(wstring saveFolder, wstring fileName, D3DXVECTOR3 scale)
{
	ReadBoneData(scene->GetRootNode(), -1, -1);
	ReadSkinData();

	WriteMeshData(saveFolder, fileName, scale);
}
	...
	for (FbxMeshData* meshData : meshDatas)
	{
		...
		for (int i = 0; i < deformerCount; i++)
		{
			...
			for (int clusterIndex = 0; clusterIndex < skin->GetClusterCount(); clusterIndex++)
			{
				...
				// 영향을 받을 인덱스 번호가 GetControlPointIndicesCount 에 들어있다..
...
void Fbx::Exporter::WriteMeshData(wstring saveFolder, wstring fileName, D3DXVECTOR3 scale)
{
	...
	for (FbxBoneData* bone : boneDatas)
	{
		...
		w->Matrix(bone->LocalTransform * S);
		w->Matrix(bone->GlobalTransform * S);
...
Fbx::FbxClip* Fbx::Exporter::ReadAnimationData(wstring name)
{
	...
	for (int i = 0; i < importer->GetAnimStackCount(); i++)
	{
		FbxTakeInfo* takeInfo = importer->GetTakeInfo(i); // 애니메이션 정보가 여기있음.
...
Fbx::FbxClip* Fbx::Exporter::ReadAnimationData(UINT index)
{
	...
	// 동작 하나를 클립.
	FbxTakeInfo* takeInfo = importer->GetTakeInfo(index); // 애니메이션 정보가 여기있음.
	clip->Name = takeInfo->mName.Buffer();

	FbxTimeSpan span = takeInfo->mLocalTimeSpan; // 애니메이션의 시간 간격
	FbxTime tempDuration = span.GetDuration();

	int start = (int)span.GetStart().GetFrameCount(); // 시작시간은 0이 아닐 수 있다. 슈발...
	...
	clip->FrameCount = (stop - start) + 1; //0부터 시작이라서.
