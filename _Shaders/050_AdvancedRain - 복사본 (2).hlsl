...
struct VSParticleIn
{
    float4 pos : POSITION0; //position of the particle
    float3 seed : SEED0;
    float3 speed : SPEED0;
    float random : RAND0;
...
void GSRenderRain(point VSParticleIn input[1], inout TriangleStream<PSSceneIn> SpriteStream)
{
    ...
    if (!cullSprite(input[0].pos.xyz, 2 * g_SpriteSize) && totalIntensity > 0)
    {
