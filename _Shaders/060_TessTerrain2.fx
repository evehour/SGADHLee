...
struct VertexOutput
{
    ...
    int Tessel : TessAmount0;
    int TesselInside : TessInside0;
};
...
VertexOutput VS(VertexTextureNormal input)
{
    ...
    output.Tessel = ComputeTesselAmount(output.Position.xyz);
    output.TesselInside = output.Tessel;
...
ConstantOutput HS_Constant(InputPatch<VertexOutput, 4> input, uint patchID : SV_PrimitiveID)
{
    ...
    output.Edges[0] = (input[0].Tessel + input[1].Tessel) * 0.5f;
    output.Edges[1] = (input[1].Tessel + input[3].Tessel) * 0.5f;
    output.Edges[2] = (input[2].Tessel + input[3].Tessel) * 0.5f;
    output.Edges[3] = (input[0].Tessel + input[2].Tessel) * 0.5f;
