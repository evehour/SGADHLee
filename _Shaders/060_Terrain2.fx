...
float CalcTessFactor(float3 position)
{
    ...
    return pow(2.0f, lerp(MaxTessellation, MinTessellation, s));
}
...
DomainOutput DS(ConstantOutput input, float2 uv : SV_DomainLocation, const OutputPatch<HullOutput, 4> patch)
{
    ...
    output.Uv = lerp(
        ...
        lerp(patch[2].Uv, patch[3].Uv, uv.x),
        uv.y);
...
float4 PS(DomainOutput input, uniform bool fogEnabled) : SV_TARGET
{
 ...
 //   float4 A, D, S;
   //ComputeDirectionalLight(
    ...
    float4 color = c0;
    //color = lerp(color, c1, t.rrrr);
    //color = lerp(color, c2, t.gggg);
    //color = lerp(color, c3, t.bbbb);
    //color = lerp(color, c4, t.aaaa);
