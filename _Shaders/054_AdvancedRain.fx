...
VertexRain VS(VertexRain input)
{
    ...
    //move forward
    //output.Position.w = 1;
    output.Position.xyz += input.Speed / g_FrameRate + g_TotalVel;
    
    //if the particle is outside the bounds, move it to random position near the eye
    [branch]
    if (output.Position.y <= ViewPosition.y - g_heightRange)
    {
        //float x = output.Seed.x + ViewPosition.x;
        //float z = output.Seed.z + ViewPosition.z;
        //float y = output.Seed.y + ViewPosition.y;
        //output.Position.xyz = float3(x, y, z);
        output.Position.xyz = output.Seed + ViewPosition;
    }
...
void GS(point VertexRain input[1], inout TriangleStream<PSSceneIn> SpriteStream)
{
    ...
    if (!cullSprite(input[0].Position.xyz, 2.0f * g_SpriteSize) && totalIntensity > 0)
    {
        ...
        float3 GLightPosition = -LightDirection * 1000.0f;
        float3 pos[4];
        ...
        output.Position = mul(output.Position, Projection);
        output.LightDir = GLightPosition - pos[0];
        ...
        output.Position = mul(output.Position, Projection);
        output.LightDir = GLightPosition - pos[1];
        ...
        output.Position = mul(output.Position, Projection);
        output.LightDir = GLightPosition - pos[2];
        ...
        output.Position = mul(output.Position, Projection);
        output.LightDir = GLightPosition - pos[3];
...
float3 calculateAirLightPointLight(float Dvp, float Dsv, float3 S, float3 V)
{
    ...
    float4 f1 = Ftable.SampleLevel(samLinearClamp, float2((v1 - g_fXOffset) * g_fXScale, (u - g_fYOffset) * g_fYScale), 0);
    float4 f2 = Ftable.SampleLevel(samLinearClamp, float2((v2 - g_fXOffset) * g_fXScale, (u - g_fYOffset) * g_fYScale), 0);
    float4 airlight = (g_beta.x * lightIntensity * exp(-g_beta.x * Dsv * cosGamma)) / (2 * PI * Dsv * sinGamma) * (f1 - f2);
...
float4 PSSurface(VSSurfaceOut input) : SV_TARGET
{
    ...
    //Fog
    float3 viewVec = input.wPosition.xyz - ViewPosition;
    ...
    float3 g_VecPointLightEye = g_PointLightPos - ViewPosition;
    float3 g_VecPointLightEye2 = g_PointLightPos2 - ViewPosition;
    ...
    //airlight
    float3 SDir = normalize(GLightPosition - ViewPosition);
    ...
    material.DiffuseColor = DiffuseMap.Sample(TrilinearSampler, input.Uv) + diffuse;

    color = float4(Lighting(LightingDatas[0], input.wPosition.xyz, ViewPosition, material), 1);
