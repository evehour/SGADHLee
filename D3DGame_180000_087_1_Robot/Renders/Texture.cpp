...
Texture::Texture(D3DXCOLOR color, D3DX11_IMAGE_LOAD_INFO * loadInfo)
{
	D3DDesc Ddesc;
	D3D::GetDesc(&Ddesc);

	D3DX11_IMAGE_LOAD_INFO defaultInfo;
	ZeroMemory(&defaultInfo, sizeof(D3DX11_IMAGE_LOAD_INFO));
	defaultInfo.Width = Ddesc.Width;
	defaultInfo.Height = Ddesc.Height;
	defaultInfo.MipLevels = 1;
	defaultInfo.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	defaultInfo.Usage = D3D11_USAGE_STAGING;
	defaultInfo.CpuAccessFlags = D3D11_CPU_ACCESS_WRITE;
	defaultInfo.BindFlags = D3D11_BIND_SHADER_RESOURCE;

	if (loadInfo == NULL)
		*loadInfo = defaultInfo;

	UINT initColor = 0;
	initColor += (UINT)(color.r * 255) << 24;
	initColor += (UINT)(color.g * 255) << 16;
	initColor += (UINT)(color.b * 255) << 8;
	initColor += (UINT)(color.a * 255);

	D3D11_SUBRESOURCE_DATA initData = { 0 };
	initData.pSysMem = &initColor;
	initData.SysMemPitch = sizeof(UINT);
	initData.SysMemSlicePitch = 0;

	ID3D11Texture2D* texture;
	D3D11_TEXTURE2D_DESC desc = { 0 };
	desc.Width = loadInfo->Width;
	desc.Height = loadInfo->Height;
	desc.Format = loadInfo->Format;
	desc.SampleDesc.Count = 1;
	desc.Usage = loadInfo->Usage;
	desc.BindFlags = loadInfo->BindFlags;
	desc.CPUAccessFlags = loadInfo->CpuAccessFlags;
	desc.MipLevels = loadInfo->MipLevels;
	desc.ArraySize = 1;

		
	HRESULT hr = D3D::GetDevice()->CreateTexture2D(&desc, &initData, &texture);
	assert(SUCCEEDED(hr));

	D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
	srvDesc.Format = loadInfo->Format;
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MipLevels = loadInfo->MipLevels;

	hr = D3D::GetDevice()->CreateShaderResourceView(texture, &srvDesc, &view);
	assert(SUCCEEDED(hr));
...
vector<ID3D11Texture2D*> TextureArray::CreateTextures(vector<wstring>& names, UINT width, UINT height, UINT mipLevels)
{
	...
	for (UINT index = 0; index < returnTextures.size(); index++)
	{
		...
		{
			hr = GetMetadataFromWICFile(names[index].c_str(), WIC_FLAGS_FORCE_RGB | WIC_FLAGS_IGNORE_SRGB, metaData);
			assert(SUCCEEDED(hr));
		}
