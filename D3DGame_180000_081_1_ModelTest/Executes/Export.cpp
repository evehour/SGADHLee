...
Export::Export()
{
	...
	////Stanford Bunny
	//exporter = new Fbx::Exporter(Assets + L"Meshes/StanfordBunny.fbx");
	//exporter->ExportMaterial(Materials + L"/Meshes/", L"StanfordBunny.material");
	//exporter->ExportMesh(Models + L"/Meshes/", L"StanfordBunny.mesh");
	//SAFE_DELETE(exporter);
	...
	exporter->ExportMaterial(Models + L"Soldier/", L"Soldier.material");
	exporter->ExportMesh(Models + L"Soldier/", L"Soldier.mesh");
	SAFE_DELETE(exporter);

	//Running
	exporter = new Fbx::Exporter(Assets + L"Soldier/demo_combat_idle.fbx");
	exporter->ExportAnimation(Models + L"Soldier/", L"demo_combat_idle.anim", 0);
	SAFE_DELETE(exporter);
}
