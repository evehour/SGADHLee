...
void Path::GetSubfolderInDirectory(vector<wstring>* folders, wstring path)
{
	wstring file;
	if (path.back() != L'/')
		file = L"/";
...
void Path::GetFiles(vector<wstring>* files, wstring path, wstring filter, bool bFindSubFolder)
{
	...
	if (handle != INVALID_HANDLE_VALUE)
	{
		...
		{
			vector<wstring> lstDir;
			GetSubfolderInDirectory(&lstDir, path);
			int nFolderCount = lstDir.size();
			for (int i = 0; i < nFolderCount; i++)
			{
				GetFiles(files, lstDir[i], filter, bFindSubFolder);
			}
		}
	}
}
