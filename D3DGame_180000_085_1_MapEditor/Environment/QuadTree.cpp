...
void QuadTree::Build(D3DXVECTOR2 & center, float width)
{
	...
	float half = (float)((int)this->width / 2);
	...
	int endZ = (int)(center.y + half);
	int endX = (int)(center.x + half);

	indexCount = (UINT)(width * width * 4);
	indices = new UINT[indexCount];
	for (int z = (int)(center.y - half); z < endZ; z++)
	{
		for (int x = (int)(center.x - half); x < endX; x++)
		{
