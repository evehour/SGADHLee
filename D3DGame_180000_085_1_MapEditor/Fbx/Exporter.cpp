...
Fbx::Exporter::Exporter(wstring file, bool bExtractTexture)
{
	manager = FbxManager::Create();
	scene = FbxScene::Create(manager, "");

	ios = FbxIOSettings::Create(manager, IOSROOT);
	ios->SetBoolProp(IMP_FBX_TEXTURE, bExtractTexture);
...
void Fbx::Exporter::WriteMeshData(wstring saveFolder, wstring fileName)
{
	...
	for (FbxMeshData* meshData : meshDatas)
	{
		...
		for (FbxMeshPartData* part : meshData->MeshParts)
		{
			...
			//part->Vertices[0].Position = D3DXVECTOR3(-0.5f, -0.5f, 0);
			//part->Vertices[1].Position = D3DXVECTOR3(-0.5f, +0.5f, 0);
			//part->Vertices[2].Position = D3DXVECTOR3(+0.5f, -0.5f, 0);
			//part->Vertices[3].Position = D3DXVECTOR3(+0.5f, -0.5f, 0);
			//part->Vertices[4].Position = D3DXVECTOR3(-0.5f, +0.5f, 0);
			//part->Vertices[5].Position = D3DXVECTOR3(+0.5f, +0.5f, 0);
...
bool Fbx::Exporter::CheckBoneData(FbxNode * node)
{
	FbxNodeAttribute* attribute = node->GetNodeAttribute();
	...
	for (int i = 0; i < node->GetChildCount(); i++)
	{
		if(CheckBoneData(node->GetChild(i))) return true;
	}

	return false;
}

UINT Fbx::Exporter::CheckIncludeData(vector<string>& animationNames)
{
	UINT retVal = 0;
	int animCount = importer->GetAnimStackCount();

	if (scene->GetMaterialCount() > 0) retVal += (UINT)FBX_INCLUDE_DATA::FBX_Material;
	if (CheckBoneData(scene->GetRootNode()))
	{
		retVal += (UINT)FBX_INCLUDE_DATA::FBX_Mesh_Bone;
	}
	if (animCount > 0)
	{
		...
		for (int i = 0; i < animCount; i++)
		{
			...
			if(takeInfo)
				animationNames.push_back(takeInfo->mName.Buffer());
		}

		if(animationNames.size() > 0)
			retVal += (UINT)FBX_INCLUDE_DATA::FBX_Animation;
	}
	
	if ((retVal & (UINT)FBX_INCLUDE_DATA::FBX_Mesh_Bone) && (retVal & (UINT)FBX_INCLUDE_DATA::FBX_Material))
		retVal += (UINT)FBX_INCLUDE_DATA::FBX_Model;
