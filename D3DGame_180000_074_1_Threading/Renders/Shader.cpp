...
void Shader::CreateEffect()
{
...
#if 0
	flag = D3D10_SHADER_ENABLE_STRICTNESS;
#else
	/*
	D3D10_SHADER_ENABLE_BACKWARDS_COMPATIBILITY
		이전버전의 쉐이더와의 호환성을 유지시켜주는 옵션인데
		정적변수의 편집을 가능하게 해줌.
	...
	*/
	...
	HRESULT hr = D3DCompileFromFile(file.c_str(), NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, NULL, "fx_5_0", flag, flag2, &fxBlob, &error);
	if (FAILED(hr))
	{
		if (error != NULL)
		{
