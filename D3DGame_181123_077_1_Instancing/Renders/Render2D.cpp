...
void Render2D::Render()
{
	...
	// 드로잉  구간
	{
		...
		Context::Get()->ChangeView(shader, view);
		Context::Get()->ChangePerspective(shader, orthographic);
	...
	{
		//Context::Get()->ChangeView(shader, v);
		//Context::Get()->ChangePerspective(shader, p);
	}
