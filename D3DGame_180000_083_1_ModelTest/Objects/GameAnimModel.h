...
public:
	...
	GameAnimModel(const GameAnimModel* T);
	...
	UINT AddClip(ModelClip* clip);

	UINT DelClip(ModelClip* clip);
	UINT DelClip(wstring name);

	void RemoveAllClip(bool inCase = true);

	int ContainClip(ModelClip* clip);
	...
	float GetClipDuration(ModelClip * clip);
