...
{
	quadShader = new Shader(L"030_Terrain");
	material = new Material(quadShader);
	//material = new Material(Shaders + L"034_NormalMapDepth.hlsl");
	//material->SetDiffuseMap(Textures + L"Dirt.png");
	material->SetDiffuseMap(Textures + L"Dirt01.png");
	material->SetNormalMap(Textures + L"Dirt01_normal.png");
	material->SetDetailMap(Textures + L"Dirt01_detail.png");
	
	worldBuffer = new WorldBuffer();
	lineBuffer = new LineBuffer();


	//perspective = new Perspective(620, 480, Math::PI * 0.25f, 1, 1000);
...
void QuadTree::Render()
{
	...
	ImGui::Begin("QuadTree");
	{
		...
		ImGui::Checkbox("QTFrustum view", &bViewFrustumArea);
		ImGui::Checkbox("QTLevelGrid view", &bViewQuadViewLevel);
	}
	ImGui::End();
...
void QuadTree::RenderNode(NodeType * node)
{
	...
	if (bViewQuadViewLevel)
	{
		if (node->Width < parent->Width)
		{
			D3DXVECTOR3 vMin, vMax, vDcenter(0, 5.0f, 0);
			D3DXCOLOR c(1, 1, 1, 1);
			float d = node->Width / 2.0f;
			...
			vMin = D3DXVECTOR3(vDcenter.x - d, -vDcenter.y, vDcenter.z - d) + center;
			vMax = D3DXVECTOR3(vDcenter.x + d,  vDcenter.y, vDcenter.z + d) + center;

			lineMake->SetColor(c);
			lineMake->MakeAABB(vMin, vMax);
			lineMake->Render();
		}
	}
