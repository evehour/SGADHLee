	...
	:values(values)
{
	...
	depthShadow = new RenderTarget(values, 1024, 768);
	...
	buffer = new Buffer();
	buffer->Data.Size = D3DXVECTOR2(1024, 768);
}
...
void Shadow::Render()
{
	...
	//samplerState[0]->PSSetSamplers(12);

	view = depthShadow->GetSRV();
	depthRender->SRV(view);
	depthRender->Update();
	depthRender->Render();
